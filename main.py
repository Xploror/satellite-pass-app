...
def main():
    ...
    conf_file_path = (sys.argv[1] if len(sys.argv) > 1 else None) or CONF_PATH.strip() #conf_file path PRIORITY: 1. CLI  2. ENV_VAR
    ...
    port = port_cli or (PORT if is_port_available(HOST, PORT) else 8080)  # Checks if host/port address is available, if not then use port_cli  
